...
def train(data,
    ...
    for step in range(trained_step, max_steps):
        ...
        # make shuffled batch
        conn_idx = np.random.randint(0, len(conn_list[0]), batch_size)
        t_idx = np.random.randint(0, num_data, batch_size)
        x0 = data[t_idx, :, conn_list[0][conn_idx]]
        x1 = data[t_idx, :, conn_list[1][conn_idx]]
        x_batch = np.stack([x0, x1], axis=1)
        y_batch = conn_idx
